...
def register_user():
    ...
    try:
        ...
        otp_expiry = datetime.now() + timedelta(minutes=5)  # 5 minutes expiry
        ...
        if firebase_client.root_ref:  # REAL MODE
            # Send via Termii SMS
            ...
            if sms_result.get('status') != 'success':
                ...
                # Still continue, OTP will be shown in response for testing
        else:  # MOCK MODE
            print(f"🔧 MOCK MODE: OTP for {email} is {otp_code}")
        ...
        return jsonify({
            ...
            'data': {
                ...
                'mock_otp': otp_code if not firebase_client.root_ref else None  # Only in mock mode
            }
        }), 201
...
def resend_otp():
    ...
    try:
        ...
        if firebase_client.root_ref:  # REAL MODE
            ...
            print(f"🔧 MOCK MODE: New OTP for {user['email']} is {new_otp}")
...
def test_database():
    try:
        ...
        user_id = str(uuid.uuid4())
        success = firebase_client.create_user(user_id, test_user)
...
def withdraw_profit():
    ...
    try:
        ...
        if not verification_result.get('status'):
            return jsonify({
                'status': 'error',
                'message': f'Bank account verification failed: {verification_result.get("message")}'
            }), 400
        
        # Update account name with verified name
        bank_details['verified_account_name'] = verification_result.get('account_name')
...
@app.route('/api/bank/verify', methods=['POST'])
def verify_bank():
    ...
    try:
        ...
        if not account_number or not bank_name:
            return jsonify({'status': 'error', 'message': 'Account number and bank name required'}), 400
        
        # Get bank code
        bank_code = get_bank_code(bank_name)
        if not bank_code:
            return jsonify({'status': 'error', 'message': 'Invalid bank name'}), 400
...
def verify_bank_account(account_number, bank_name):
    """Verify bank account using Paystack"""
    try:
        # First get bank code from bank name
        bank_code = get_bank_code(bank_name)
        
        if not bank_code:
            return {'status': False, 'message': 'Invalid bank name'}
        ...
        if result.get('status'):
            ...
            return {'status': False, 'message': result.get('message', 'Account verification failed')}
