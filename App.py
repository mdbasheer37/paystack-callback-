...
class FirebaseClient:
    ...
    def __init__(self):
        try:
            ...
            if firebase_config:
                ...
                print("⚠️ Firebase config not found, running in mock mode")
                self.root_ref = None
    ...
    def create_profit_ledger_entry(self, ledger_data):
...
class PaystackService:
    def __init__(self):
        ...
        if not self.secret_key or self.secret_key == 'sk_test_xxxxxxxxxxxxxx':
            print("⚠️ Paystack secret key not configured - payments will fail")
        else:
...
class VTPassService:
    def __init__(self):
        ...
        if not self.api_key:
            print("⚠️ VTPass API key not configured - VTPass services will fail")
        else:
            print("✅ VTPass Service Initialized")
    ...
    def pay(self, service_id, billers_code, variation_code, amount, phone=None):
        ...
        if not self.api_key:
            return {'code': '099', 'response_description': 'VTPass not configured'}
...
def process_bank_transfer(amount, bank_details):
    ...
    try:
        ...
        # You'll need to implement actual Paystack transfer later
