...
class PricingConfig:
    ...
    @staticmethod
    def calculate_selling_price(base_price: float, service_type: str) -> float:
    ...
    @staticmethod
    def calculate_profit_amount(selling_price: float, base_price: float, service_type: str) -> Tuple[float, float]:
        """Calculate profit with minimum profit guarantee"""
...
class FirebaseClient:
    ...
    def __init__(self):
        try:
            ...
            print(f"⚠️ Firebase initialization failed: {str(e)} - Using mock mode")
            self.root_ref = None
    ...
    def get_transaction_by_reference(self, reference: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            if self.root_ref:
                transactions = self.root_ref.child('transactions').get() or {}
            else:
    ...
    def get_profit_data(self) -> Dict[str, Any]:
    ...
    def get_otp_record(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            if self.root_ref:
                otp_records = self.root_ref.child('otp_records').get() or {}
            else:
                otp_records = self.mock_otp_records
    ...
    def update_otp_record(self, user_id: str, updates: Dict[str, Any]) -> bool:
        ...
        try:
            if self.root_ref:
                ...
                for otp_id, otp_data in otp_records.items():
                    if otp_data.get('user_id') == user_id:
                        self.root_ref.child(f'otp_records/{otp_id}').update(updates)
                        return True
                return False
            else:
...
class VTPassService:
    ...
    def make_request(self, endpoint: str, method: str = "POST", data: Dict = None) -> Dict[str, Any]:
    ...
    def pay(self, service_id: str, billers_code: str, variation_code: str, 
            amount: float, phone: str = None) -> Dict[str, Any]:
    ...
    def verify_service(self, service_id: str, billers_code: str) -> Dict[str, Any]:
...
# ==================== RESPONSE HANDLER WITH PROFIT ====================

def handle_vtpass_response_with_profit(result: Dict, request_data: Dict, service_type: str, 
                                       base_amount: float, selling_amount: float, 
                                       expected_profit: float) -> Tuple[Dict, int]:
    ...
    try:
        ...
        if 'user' in locals() and user and transaction_deducted:
            firebase_client.update_user_wallet(user['id'], selling_amount)
        
        print(f"💥 Profit handling error: {str(e)}")
        return {'status': 'error', 'message': f'Transaction processing failed: {str(e)}'}, 500
...
def purchase_airtime():
    ...
    try:
        ...
        # Handle response
        return jsonify(handle_vtpass_response_with_profit(
...
def process_bank_transfer(amount: float, bank_details: Dict) -> Dict:
    """Process bank transfer"""
    try:
        # For now, return mock success
        print(f"💰 Bank transfer: ₦{amount} to {bank_details.get('account_name')}")
