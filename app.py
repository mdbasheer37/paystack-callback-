...
class Config:
    ...
    ADMIN_EMAILS = ['admin@cheap4u.com', 'your_email@gmail.com']
...
def calculate_profit(service_type, amount, service_details=None):
    ...
    if service_type == 'data':
        # Data bundles - fixed profit per GB
        plan = service_details.get('plan', '')
        if '1GB' in plan:
            return 30.0  # ₦30 profit per 1GB
        elif '2GB' in plan:
            return 50.0  # ₦50 profit per 2GB
        else:
            return amount * 0.05  # 5% for other data plans
            
    elif service_type == 'airtime':
        return amount * 0.02  # 2% for airtime
        
    elif service_type == 'electricity':
        return amount * 0.02  # 2% for electricity
        
    elif service_type == 'cable_tv':
        return amount * 0.05  # 5% for cable TV
        
    elif service_type == 'exam_pins':
        exam_type = service_details.get('exam_type', '')
        if 'JAMB' in exam_type:
            return 500.0
        else:
            return 300.0  # WAEC, NECO, NABTEB
    
    else:
        return amount * 0.03  # 3% default profit
...
def vtpass_pay():
    """Enhanced VTPass payment with profit tracking"""
    try:
        data = request.get_json()
        if not data:
            return jsonify({'status': 'error', 'message': 'No JSON data provided'}), 400
        ...
        profit_amount = calculate_profit(service_type, amount, data)
